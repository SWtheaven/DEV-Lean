(() => {
  ...
  const config = window.LUCRONOMIA_CONFIRMA_CONFIG || {};
  ...
  const STORAGE = Object.freeze({
    events: "lucronomia_confirma_events_v0",
    draft: "lucronomia_confirma_draft_v0",
    checkoutOpened: "lucronomia_confirma_checkout_opened_v0"
  });
  ...
  function track(eventName, properties = {}) {
    const event = {
      ...
      product: config.product || "LucronomIA Confirma",
      path: window.location.pathname,
      ...properties
    };

    try {
      const current = JSON.parse(localStorage.getItem(STORAGE.events) || "[]");
      current.push(event);
      localStorage.setItem(STORAGE.events, JSON.stringify(current.slice(-100)));
    } catch (_) {
  ...
  function validate() {
    ...
    formError.textContent = valid ? "" : "Preencha os campos obrigatórios para gerar o resumo.";
  ...
  function persistDraft(data) {
    try {
      sessionStorage.setItem(STORAGE.draft, JSON.stringify(data));
    } catch (_) {
  ...
  function setCheckoutAvailability() {
    if (checkoutReady()) {
      ...
      checkoutStatus.textContent = "Checkout Mercado Pago aguardando ativação do link de pagamento.";
    }
  }

  checkoutButton.addEventListener("click", () => {
    if (!checkoutReady() || !state.data) return;
    state.checkoutOpened = true;
    persistDraft(state.data);
    try {
      sessionStorage.setItem(STORAGE.checkoutOpened, "1");
    } catch (_) {
      // The product still works if sessionStorage is unavailable.
    }
    track("checkout_click", {
      price: config.price || "R$ 9,90",
      checkout_provider: config.checkoutProvider || "Mercado Pago — Link de Pagamento"
    });
    window.location.assign(config.checkoutUrl.trim());
  });

  paymentDoneButton.addEventListener("click", () => {
    if (!state.checkoutOpened || !state.data) return;
    ...
    try {
      sessionStorage.removeItem(STORAGE.checkoutOpened);
    } catch (_) {}
  ...
  function restoreCheckoutSession() {
    let checkoutWasOpened = false;
    try {
      checkoutWasOpened = sessionStorage.getItem(STORAGE.checkoutOpened) === "1";
    } catch (_) {}
    if (!checkoutWasOpened) return;

    const draft = readDraft();
    if (!draft || !draft.service || !draft.clientName) return;
    ...
    checkoutStatus.textContent = "Voltou do Mercado Pago? Confirme abaixo após concluir o pagamento.";
  }
  ...
  whatsappButton.addEventListener("click", () => {
    ...
    window.location.href = `https://wa.me/?text=${encodeURIComponent(whatsappMessage(state.data))}`;
  });
  ...
  function wrapText(text, maxChars) {
    ...
    String(text || "").split(/\n/).forEach((paragraph, paragraphIndex, paragraphs) => {
      ...
      if (current) output.push(current);
      if (!words.length || paragraphIndex < paragraphs.length - 1) output.push("");
    });
    return output;
  }
  ...
  function buildPdf(data) {
    ...
    lines.push({ text: "LucronomIA Confirma", size: 15, bold: true, color: "0 0.55 0.30", gap: 25 });
