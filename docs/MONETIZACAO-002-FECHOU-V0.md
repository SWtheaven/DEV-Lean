...
Aplicação estática em `products/fechou/`:
...
A V0 pode ser hospedada em solução estática gratuita, sem custo fixo adicional de infraestrutura.

## Checkout

O checkout é externo e propositalmente simples. Para ativar vendas, preencher `checkoutUrl` em `products/fechou/config.js` com uma URL HTTPS real do provedor escolhido.
