(() => {
  ...
  function renderBalance() {
    $("#wallet-pill").textContent = `Saldo: ${wallet.balance} confirmação${wallet.balance === 1 ? "" : "ões"}`;
  }
  ...
  async function buy(packageCode) {
    try {
      ...
      if (error.code === "MP_NOT_CONFIGURED") {
        alert("Checkout Mercado Pago ainda aguarda as credenciais server-side para o teste real.");
      } else {
