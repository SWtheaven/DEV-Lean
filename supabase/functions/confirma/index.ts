...
const PUBLIC_APP_URL = "https://swtheaven.github.io/DEV-Lean/confirma/";
const FUNCTION_ROOT = "/functions/v1/confirma";
...
function empty(status = 204) {
  return new Response(null, { status, headers: { ...corsHeaders, ...securityHeaders } });
}
...
async function validateMpSignature(req, url) {
...
Deno.serve(async (req) => {
  try {
    ...
    if (req.method === "GET" && path === "/styles.css") {
      return Response.redirect(`${PUBLIC_APP_URL}styles.css`, 302);
    }
    if (req.method === "GET" && path === "/app.js") {
      return Response.redirect(`${PUBLIC_APP_URL}app.js`, 302);
    }
    ...
    if (req.method === "POST" && path === "/api/wallet") {
      ...
      const { data, error } = await supabase
        .from("confirma_wallets")
        .insert({ access_token_hash: await sha256Hex(token) })
        .select("id,balance,created_at")
        .single();
      if (error) throw error;
    ...
    if (req.method === "POST" && path === "/api/checkout") {
      const wallet = await walletFromRequest(req);
      if (!wallet) return json({ error: "WALLET_UNAUTHORIZED" }, 401);
      if (!MP_ACCESS_TOKEN) return json({ error: "MP_NOT_CONFIGURED" }, 503);

      const body = await req.json().catch(() => ({}));
      const packageCode = String(body.package_code || "");
      const { data: pkg } = await supabase
        .from("confirma_packages")
        .select("code,label,credits,price_cents,description,static_checkout_url")
        .eq("code", packageCode)
        .eq("active", true)
        .maybeSingle();
      ...
      const { data: order, error: orderError } = await supabase
        .from("confirma_orders")
        .insert({
          wallet_id: wallet.id,
          package_code: pkg.code,
          expected_price_cents: pkg.price_cents,
          expected_credits: pkg.credits
        })
        .select("id")
        .single();
      if (orderError) throw orderError;

      const returnUrl = `${PUBLIC_APP_URL}?payment_return=1&order=${order.id}`;
      const notificationUrl = `${SUPABASE_URL}${FUNCTION_ROOT}/api/webhook`;
      const preferencePayload = {
        items: [{
          id: pkg.code,
          title: `LucronomIA Confirma — ${pkg.label}`,
      ...
      await supabase.from("confirma_events").insert({
        ...
        properties: { order_id: order.id, package_code: pkg.code }
      });
    ...
    if (req.method === "GET" && path === "/api/payment-status") {
      ...
      const { data: order } = await supabase
        .from("confirma_orders")
        .select("id,status,package_code,expected_credits,approved_at")
        .eq("id", orderId)
    ...
    if (req.method === "POST" && path === "/api/finalize") {
      ...
      if (clientId.length < 8 || clientId.length > 120 || !payload || typeof payload !== "object") {
        return json({ error: "INVALID_CONFIRMATION" }, 400);
      }
      const { data, error } = await supabase.rpc("confirma_finalize_confirmation", {
        p_wallet_id: wallet.id,
        p_client_confirmation_id: clientId,
        p_payload: payload
      });
      if (error) {
        if (String(error.message).includes("INSUFFICIENT_CREDITS")) {
          return json({ error: "INSUFFICIENT_CREDITS" }, 409);
        }
        throw error;
      }
      const result = data?.[0] || {};
      await supabase.from("confirma_events").insert({
        wallet_id: wallet.id,
        event_name: "confirmation_finalized",
        properties: {
    ...
    if (req.method === "POST" && path === "/api/webhook") {
      ...
      const { data: order } = await supabase
        .from("confirma_orders")
        .select("id,wallet_id,expected_price_cents,expected_credits,package_code,status")
        .eq("id", orderId)
        .maybeSingle();
      if (!order) return json({ error: "ORDER_NOT_FOUND" }, 404);

      const amountCents = Math.round(Number(payment.transaction_amount || 0) * 100);
      if (String(payment.currency_id || "") !== "BRL" || amountCents !== order.expected_price_cents) {
        return json({ error: "PAYMENT_MISMATCH" }, 409);
      }

      if (String(payment.status || "") !== "approved") {
        const mapped = ["rejected", "cancelled"].includes(String(payment.status))
      ...
      const { data, error } = await supabase.rpc("confirma_apply_approved_payment", {
        p_order_id: order.id,
